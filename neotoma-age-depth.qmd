...
- sample the dataset to reduce the number of rows(`Sluggan2 <- Sluggan[c(1, 10, 31), ]`)
...
So far we have used the built-in dataset from the northern hemisphere for the sake of demonstration. But let's now follow an example by pulling data from the neotoma database. Although this is not a `neotoma2` tutorial, we can cover rebuilding a chronology if we still have time.

The following code will download data from the neotoma database. I have also saved some example datasets in the data directory just in case the neotoma API is under maintenance or there are download speed issues. If you have your own data or would like to download a site of your choice from neotoma feel free to do that. [neotoma explorer](https://apps.neotomadb.org/explorer/index.html) can help you find a site id.


site IDs:

- Laguna Padre Laguna <- 10063 (Argentina)
- Mallin Book <- 1639 (Argentina)
- Harberton <- 10136 ()
- Lago Puerto Arturo <- 28348 (Guatemala)
- Albatross Lake <- 28408 (Marion Island)
- Lake Edward <- 26660
- Lake Victoria II <- 31747 (Uganda linear interpolation)
- Lake Kirkpatrick <- 9865
- Lake Frome <- 28489 (Auestrailia)
...
newChron <- Bchron::Bchronology(
  ...
  calCurves = c("normal", rep("intcal20", 9)),
