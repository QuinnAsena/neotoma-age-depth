...
pacman::p_load(neotoma2, Bchron, splines) # splines comes with BASE-R
```
:::
...
So far we have used the built-in datasets for the sake of demonstration. But let's now follow a real example by pulling data from the neotoma database.

The following code will download data from neotoma.....  I have also saved some example datasets in the data directory just in case the neotoma API is under maintainance or there are download speed issues.
